...
It relies on temporary env-driven force-routing hooks in `w4a8_grouped_mm_c3x.cu`
so we can compare candidate kernels without changing the production dispatch.
"""
...
def main() -> None:
    ...
    for shape_index, shape_name in enumerate(requested_shapes):
    ...
    print(markdown)
    print(f"Benchmark finished in {total_s:.1f}s", flush=True)
