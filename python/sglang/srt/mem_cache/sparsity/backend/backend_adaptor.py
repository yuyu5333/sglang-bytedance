        ...
        **kwargs,
        ...
        if True:
            ...
            if req_pool_indices.numel() > 0:
                ...
                print(f"[DEBUG] [adapt_for_attn_metadata] Comparing page_table_dense[0] vs page_table_pool[{req_idx-1}]:")
                print(f"  page_table_dense: {page_table_dense}")
                print(f"  page_table_pool: {page_table_pool}")
                print(f"  Are they equal? {torch.allclose(page_table_dense[0], page_table_pool[req_idx-1])}")
