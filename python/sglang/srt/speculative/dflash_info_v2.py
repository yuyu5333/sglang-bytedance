...
class DFlashDecodePrepareMixin:
    """Shared verify-block KV pre-allocation for DFLASH-family inputs.

    Under PP, non-last ranks carry :class:`DSparkPPVerifyInputRaw` instead of
    ``DFlashDraftInputV2``. They still execute the same target verify forward
    and therefore must reserve the same req-to-token slots as the last rank.
    """
