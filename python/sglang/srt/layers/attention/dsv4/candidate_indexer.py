...
def _get_deepselect_topk(
    scores: torch.Tensor,
) -> Optional[Callable[[torch.Tensor, int], tuple[torch.Tensor, torch.Tensor]]]:
    if not scores.is_cuda or get_platform().device_sm != 90:
        return None
    try:
        from sgl_kernel.deepselect import deepselect_topk_fp32
    except (ImportError, OSError):
        return None
    if not hasattr(torch.ops.sgl_kernel, "deepselect_topk_fp32"):
        return None
    return deepselect_topk_fp32
...
def _candidate_block_topk(
    ...
    if deepselect_topk is not None and _can_fuse_candidate_blocks(
        ...
        values, indices = deepselect_topk(scores, selected)
        return _CandidateTopK(values=values, indices=indices, fused_publication=True)
    ...
    if deepselect_topk is not None:
        values, indices = deepselect_topk(scores, selected)
        return _CandidateTopK(values=values, indices=indices, fused_publication=False)
