...
class DeepseekV4MultiStepBackend(DeepseekV4AttnBackend):
    ...
    def _split_out_cache_loc_by_step(self, out_cache_loc: torch.Tensor) -> torch.Tensor:
        if out_cache_loc.numel() == 0:
            return out_cache_loc.view(self.speculative_num_steps, 0)
        return out_cache_loc.reshape(-1, self.topk, self.speculative_num_steps).permute(
            2, 0, 1
        ).reshape(self.speculative_num_steps, -1)

    def init_forward_metadata(self, forward_batch: ForwardBatch):
        original_out_cache_loc = forward_batch.out_cache_loc
        split_out_cache_loc = self._split_out_cache_loc_by_step(original_out_cache_loc)
        try:
            for i in range(self.speculative_num_steps - 1):
                forward_batch.out_cache_loc = split_out_cache_loc[i]
