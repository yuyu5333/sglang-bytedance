...
void cutlass_mxfp4a8_humming_moe_core_fused_gemm1(
    torch::Tensor& c1,
    torch::Tensor& c2,
    torch::Tensor const& input,
    torch::Tensor const& topk_ids,
    ...
    torch::Tensor& row_amax,
    torch::Tensor& row_arrivals) {
  TORCH_CHECK(!expert_ids.has_value(), "fused Humming GEMM1 does not support compact experts");
  TORCH_CHECK(
      c1.sizes() == intermediate_q.sizes(),
      "fused Humming GEMM1 staging must have shape [rows, intermediate]");
  if (prepare_inputs) {
    get_cutlass_w4a8_moe_mm_data_with_permutation(
        topk_ids,
        expert_offsets,
  ...
  humming_per_token_quant_fp8_shuffled(
      ...
      expert_offsets,
      num_experts);
  cutlass_mxfp4a8_humming_fused_gemm1_sm90(
      c1,
      intermediate_q,
      a2_scale,
      row_amax,
