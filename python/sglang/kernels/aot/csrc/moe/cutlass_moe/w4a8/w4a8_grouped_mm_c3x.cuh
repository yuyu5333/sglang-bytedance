...
#include "cutlass_extensions/gemm/collective/sm90_mma_array_tma_gmma_transposed.hpp"
...
struct cutlass_3x_w4a8_group_gemm {
  ...
  static_assert(!UseTransposedMma || (UsePreMmaE8M0Scale && !UseSingleWarpgroup && cute::size(ClusterShape{}) == 1));
  ...
  using RawCollectiveMainloop = typename cutlass::gemm::collective::CollectiveBuilderMixedInput<
      ...
      ClusterShape,
      std::conditional_t<
          UseSingleWarpgroupKernel || UseTransposedMma,
          cutlass::gemm::collective::StageCount<UseTransposedMma ? 2 : 3>,
  ...
  using CollectiveMainloopScaleOnly = std::conditional_t<
      UseTransposedMma,
      cutlass::gemm::collective::TransposedMxfp4Mainloop<RawCollectiveMainloop>,
