...
struct SM90_N16_PREPARED_LUT_MXFP4 {
  using Base = SM90_SWG_MXFP4<16>::Cutlass3xW4A8Gemm;
  struct Cutlass3xW4A8Gemm : Base {
    ...
    using CollectiveMainloopScaleOnly = cutlass::gemm::collective::CollectiveMmaArrayMixedInput<
        ...
        cutlass::gemm::collective::PreparedOffsetLut,
...
template <int N>
struct SM90_SWG_EARLY_REFILL_MXFP4 {
  using BaseConfig = std::conditional_t<N == 16, SM90_N16_PREPARED_LUT_MXFP4, SM90_SWG_MXFP4<N>>;
...
struct SM90_N64_HEAVY56_MXFP4 {
  using Base = SM90_N64_INDEPENDENT_TMA_MXFP4::Cutlass3xW4A8Gemm;
  struct Cutlass3xW4A8Gemm : Base {
    static constexpr auto ExpertRows = sgl_kernel::swg_detail::ExpertRowPolicy::Above56;
  };
};
...
struct SM90_ACTIVATION_SWIZZLE_MXFP4 {
  using Base = typename BaseConfig::Cutlass3xW4A8Gemm;
  struct Cutlass3xW4A8Gemm : Base {
    using OldMainloop = typename Base::CollectiveMainloopScaleOnly;
    static_assert(!OldMainloop::SwapAB);
    static_assert(Bytes == 32 || Bytes == 64);
    using ActivationLayout = std::conditional_t<
        Bytes == 32,
        cute::GMMA::Layout_K_SW32_Atom<cutlass::float_e4m3_t>,
        cute::GMMA::Layout_K_SW64_Atom<cutlass::float_e4m3_t>>;
    using Mainloop = cutlass::gemm::collective::CollectiveMmaArrayMixedInput<
        typename OldMainloop::DispatchPolicy,
        typename OldMainloop::TileShape,
        cute::tuple<cutlass::float_e2m1_t, cutlass::float_ue8m0_t>,
...
template <class BaseConfig, int Numerator, int Denominator>
struct SM90_REDUCED_GRID_MXFP4 {
  using Base = typename BaseConfig::Cutlass3xW4A8Gemm;
  struct Cutlass3xW4A8Gemm : Base {
    using BaseKernel = typename Base::GemmKernelScaleOnly;
    static_assert(cute::size(typename BaseKernel::ClusterShape{}) == 1);
    static_assert(Numerator > 0 && Numerator < Denominator);
    struct GemmKernelScaleOnly : BaseKernel {
      static dim3 get_grid_shape(typename BaseKernel::Params const& params) {
        auto grid = BaseKernel::get_grid_shape(params);
        uint32_t const workers = grid.x * grid.y * grid.z;
        uint32_t const reduced = workers * Numerator / Denominator;
        return dim3(reduced > 0 ? reduced : 1, 1, 1);
      }
    };
    using GemmScaleOnly = cutlass::gemm::device::GemmUniversalAdapter<GemmKernelScaleOnly>;
  };
...
void dispatch_mxfp4a8_fused_moe_mm_sm90(
  ...
  switch (swg_config) {
    ...
    case 364:
      ...
      return;
    case 391:
      INVOKE_GEMM_WITH_CONFIG_AS((SM90_N16_PREPARED_LUT_MXFP4));
      return;
    case 392:
      INVOKE_GEMM_WITH_CONFIG_AS((SM90_SWG_EARLY_REFILL_MXFP4<8>));
      return;
    case 393:
    ...
    case 401:
      ...
      return;
    case 402:
      INVOKE_GEMM_WITH_CONFIG_AS((SM90_TAIL_HANDOFF_MXFP4<SM90_PRECOMPUTED_MXFP4<128, 64, 256, 1, 1, false>>));
      return;
    case 403:
      INVOKE_GEMM_WITH_CONFIG_AS((SM90_TAIL_HANDOFF_MXFP4<SM90_PRECOMPUTED_MXFP4<128, 32, 512, 1, 1, false>>));
      return;
    case 404:
      INVOKE_GEMM_WITH_CONFIG_AS((SM90_TAIL_HANDOFF_MXFP4<SM90_PRECOMPUTED_MXFP4<128, 32, 512>>));
      return;
    case 405:
    ...
    case 470:
      ...
      return;
    case 471:
      INVOKE_GEMM_WITH_CONFIG_AS((SM90_GLOBAL_ACTIVATION_TMA_MXFP4<SM90_N64_INDEPENDENT_TMA_MXFP4>));
      return;
    case 473:
      INVOKE_GEMM_WITH_CONFIG_AS(
          (SM90_GLOBAL_ACTIVATION_TMA_MXFP4<SM90_PRECOMPUTED_MXFP4_WARP_SHUFFLE_PACKED_GEMM2>));
      return;
    case 474:
      INVOKE_GEMM_WITH_CONFIG_AS(
    ...
    case 475:
      ...
      return;
    case 476:
      INVOKE_GEMM_WITH_CONFIG_AS((SM90_GLOBAL_ACTIVATION_TMA_MXFP4<SM90_PACKED_MAIN_N32_MXFP4>));
      INVOKE_GEMM_WITH_CONFIG_AS(
          (SM90_GLOBAL_ACTIVATION_TMA_MXFP4<SM90_N16_K256_SWG_MXFP4<sgl_kernel::swg_detail::ExpertRowPolicy::TailN16>>));
      return;
    case 483:
      INVOKE_GEMM_WITH_CONFIG_AS(
          (SM90_WARP_METADATA_MXFP4<SM90_N16_K256_SWG_MXFP4<sgl_kernel::swg_detail::ExpertRowPolicy::AtMost16>>));
      INVOKE_GEMM_WITH_CONFIG_AS(
    ...
    case 610:
      INVOKE_GEMM_WITH_CONFIG_AS(
          (SM90_ACTIVATION_SWIZZLE_MXFP4<SM90_GLOBAL_ACTIVATION_TMA_MXFP4<SM90_PACKED_MAIN_N32_MXFP4>, 64>));
      INVOKE_GEMM_WITH_CONFIG_AS(
          (SM90_GLOBAL_ACTIVATION_TMA_MXFP4<SM90_N16_K256_SWG_MXFP4<sgl_kernel::swg_detail::ExpertRowPolicy::TailN16>>));
      return;
    case 616:
    ...
    case 621:
      INVOKE_GEMM_WITH_CONFIG_AS(
          (SM90_REDUCED_GRID_MXFP4<SM90_GLOBAL_ACTIVATION_TMA_MXFP4<SM90_PACKED_MAIN_N32_MXFP4>, 7, 8>));
      INVOKE_GEMM_WITH_CONFIG_AS(
          (SM90_GLOBAL_ACTIVATION_TMA_MXFP4<SM90_N16_K256_SWG_MXFP4<sgl_kernel::swg_detail::ExpertRowPolicy::TailN16>>));
      return;
    case 622:
    ...
    case 623:
      INVOKE_GEMM_WITH_CONFIG_AS(
          (SM90_REDUCED_GRID_MXFP4<SM90_GLOBAL_ACTIVATION_TMA_MXFP4<SM90_PACKED_MAIN_N32_MXFP4>, 3, 4>));
      INVOKE_GEMM_WITH_CONFIG_AS(
          (SM90_GLOBAL_ACTIVATION_TMA_MXFP4<SM90_N16_K256_SWG_MXFP4<sgl_kernel::swg_detail::ExpertRowPolicy::TailN16>>));
      return;
    case 624:
    ...
    case 625:
      INVOKE_GEMM_WITH_CONFIG_AS(
          (SM90_CONSUMER_REGISTERS_MXFP4<SM90_GLOBAL_ACTIVATION_TMA_MXFP4<SM90_PACKED_MAIN_N32_MXFP4>, 192>));
      INVOKE_GEMM_WITH_CONFIG_AS(
          (SM90_GLOBAL_ACTIVATION_TMA_MXFP4<SM90_N16_K256_SWG_MXFP4<sgl_kernel::swg_detail::ExpertRowPolicy::TailN16>>));
      return;
    case 626:
    ...
    case 627:
      INVOKE_GEMM_WITH_CONFIG_AS(
          (SM90_CONSUMER_REGISTERS_MXFP4<SM90_GLOBAL_ACTIVATION_TMA_MXFP4<SM90_PACKED_MAIN_N32_MXFP4>, 176>));
      INVOKE_GEMM_WITH_CONFIG_AS(
          (SM90_GLOBAL_ACTIVATION_TMA_MXFP4<SM90_N16_K256_SWG_MXFP4<sgl_kernel::swg_detail::ExpertRowPolicy::TailN16>>));
      return;
    default:
      TORCH_CHECK(
          ...
          "603, 608, 609, 610, 616, 617, 618, 619, 620, 621, 622, 623, 624, 625, 626, 627");
  }
}
