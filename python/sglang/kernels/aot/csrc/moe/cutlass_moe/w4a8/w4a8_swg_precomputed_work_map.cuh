...
CUTLASS_HOST_DEVICE bool swg_select_expert_rows(uint64_t rows) {
  ...
  } else if constexpr (Policy == ExpertRowPolicy::MainN64Tight) {
    return rows >= 64 || rows % 64 > 48;
  } else if constexpr (Policy == ExpertRowPolicy::TailN32Of64) {
    return rows % 64 > 16 && rows % 64 <= 48;
  } else if constexpr (Policy == ExpertRowPolicy::TailN16Of64) {
    uint64_t const tail = rows % 64;
    return (tail > 0 && tail <= 16) || (tail > 32 && tail <= 48);
  }
  return true;
}
...
__device__ __forceinline__ SwgGroupInfo swg_group_info(Problem const& problem) {
  ...
  uint64_t const token_tiles = [&] {
    if constexpr (RowPolicy == ExpertRowPolicy::MainN32) {
      ...
      return uint64_t(1);
    } else if constexpr (RowPolicy == ExpertRowPolicy::MainN64Tight) {
      static_assert(TileN == 64);
      return rows / 64 + uint64_t(rows % 64 > 48);
    } else if constexpr (RowPolicy == ExpertRowPolicy::TailN32Of64) {
      static_assert(TileN == 32);
      return uint64_t(1);
    } else if constexpr (RowPolicy == ExpertRowPolicy::TailN16Of64) {
      static_assert(TileN == 16);
      return uint64_t(1);
    } else {
  ...
  static_assert(
      (RowPolicy != ExpertRowPolicy::MainN32 && RowPolicy != ExpertRowPolicy::TailN16 &&
       RowPolicy != ExpertRowPolicy::MainN64Tight && RowPolicy != ExpertRowPolicy::TailN32Of64 &&
       RowPolicy != ExpertRowPolicy::TailN16Of64) ||
...
__global__ void build_swg_precomputed_work_map_kernel(
  ...
  for (uint64_t local_tile = uint64_t(tid); local_tile < group_tiles; local_tile += uint64_t(blockDim.x)) {
    ...
    } else if constexpr (RowPolicy == ExpertRowPolicy::TailN32Of64 || RowPolicy == ExpertRowPolicy::TailN16Of64) {
      uint64_t const rows = uint64_t(cute::get<1>(problem_shapes[group]));
      uint64_t const first_tail_row =
          rows / 64 * 64 + (RowPolicy == ExpertRowPolicy::TailN16Of64 && rows % 64 > 32 ? 32 : 0);
      packed_tile = SwgWorkTile::pack(
          SwgWorkTile::channel_idx(packed_tile), first_tail_row / TileN + SwgWorkTile::token_idx(packed_tile), group);
    }
...
SwgPrecomputedWorkMap build_swg_precomputed_work_map(
  ...
  if constexpr (
      ...
      Gemm::ExpertRows == ExpertRowPolicy::MainN64Tight || Gemm::ExpertRows == ExpertRowPolicy::TailN32Of64 ||
      Gemm::ExpertRows == ExpertRowPolicy::TailN16Of64) {
