...
kvbit_mxint4_sparse_decode_fwd(
    const at::Tensor& q,
    const at::Tensor& kv,
    const at::Tensor& indices,
    const std::optional<at::Tensor>& topk_length,
...
static std::tuple<at::Tensor, at::Tensor, std::optional<at::Tensor>, std::optional<at::Tensor>>
kvbit_sint4_fp16step_sparse_decode_fwd(
    const at::Tensor& q,
    const at::Tensor& kv,
    const at::Tensor& indices,
    const std::optional<at::Tensor>& topk_length,
...
TORCH_LIBRARY_FRAGMENT(sgl_kernel, m) {
  ...
  m.def(
      "kvbit_mxint4_sparse_decode_fwd(Tensor q, Tensor kv, Tensor indices, Tensor? topk_length, "
      ...
      "Tensor? extra_packed_kcache) -> (Tensor, Tensor, Tensor?, Tensor?)");
  m.impl("kvbit_mxint4_sparse_decode_fwd", torch::kCUDA, &kvbit_mxint4_sparse_decode_fwd);
  m.def(
      "kvbit_sint4_fp16step_sparse_decode_fwd(Tensor q, Tensor kv, Tensor indices, Tensor? topk_length, "
      "Tensor? attn_sink, Tensor? tile_scheduler_metadata, Tensor? num_splits, Tensor? extra_kv, "
      "Tensor? extra_indices, Tensor? extra_topk_length, int d_v, float sm_scale, Tensor packed_kcache, "
      "Tensor? extra_packed_kcache) -> (Tensor, Tensor, Tensor?, Tensor?)");
  m.impl("kvbit_sint4_fp16step_sparse_decode_fwd", torch::kCUDA, &kvbit_sint4_fp16step_sparse_decode_fwd);
}
