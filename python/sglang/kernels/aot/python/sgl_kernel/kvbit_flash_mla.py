...
def kvbit_sint4_fp16step_flash_mla_with_kvcache(
    *,
    q: torch.Tensor,
    k_cache: torch.Tensor,
    head_dim_v: int,
    ...
    """Run the fixed 368-byte SINT4 FP16-step MODEL1/H64 specialization."""
    if _kvbit_flashmla_import_error is not None:
        raise _IMPORT_ERROR from _kvbit_flashmla_import_error
    if indices is None:
        raise ValueError(
            "KVBit SINT4 FP16-step FlashMLA requires sparse decode indices"
        )

    out, lse, new_metadata, new_num_splits = (
        torch.ops.sgl_kernel.kvbit_sint4_fp16step_sparse_decode_fwd.default(
            q,
            k_cache,
            indices,
            topk_length,
