...
class TestDSV4KVBitPackedCompressor(CustomTestCase):
    ...
    @unittest.skipUnless(torch.cuda.is_available(), "CUDA is required")
    def test_flashmla_sint4_fp16step_swa_and_extra_match_cpu_reference(self):
        from sgl_kernel.flash_mla import FlashMLASchedMeta
        from sgl_kernel.kvbit_flash_mla import (
            kvbit_sint4_fp16step_flash_mla_with_kvcache,
        )
        ...
        for extra_page_size, extra_locs in (
            ...
            with self.subTest(extra_page_size=extra_page_size):
                ...
                output, _ = kvbit_sint4_fp16step_flash_mla_with_kvcache(
                    q=q.cuda(),
                    k_cache=swa_cache.view(swa_cache.shape[0], 256, 1, 368),
                    head_dim_v=512,
                    sched_meta=FlashMLASchedMeta(),
                    softmax_scale=512**-0.5,
                    indices=swa_indices,
                    topk_length=lengths,
                    attn_sink=sink.cuda(),
                    packed_kcache=swa_cache.view(-1, 368),
                    extra_k_cache=extra_cache.view(
                        extra_cache.shape[0], extra_page_size, 1, 368
                    ),
                    extra_indices_in_kvcache=extra_indices,
                    extra_topk_length=lengths,
                    extra_packed_kcache=extra_cache.view(-1, 368),
                )
                all_keys = torch.cat(
                    (
                        decode_dsv4_sint4_fp16step_reference(
                            encode_dsv4_sint4_fp16step_reference(swa_keys)
                        ),
                        decode_dsv4_sint4_fp16step_reference(
                            encode_dsv4_sint4_fp16step_reference(extra_keys)
                        ),
                    ),
                    dim=0,
                ).float()
